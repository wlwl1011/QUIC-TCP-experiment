...
namespace ns3
{
    ...
    {
        m_socket = Socket::CreateSocket(GetNode(), TcpSocketFactory::GetTypeId()); // void QuicServerApp::Bind(uint16_t port)
        m_socket->Bind();
        m_socket->SetAttribute("SegmentSize", UintegerValue(kMSS));
        // std::cout << kMSS << std::endl;
    ...
    void TcpClient::NotifiSendData()
    {
        ...
        {
            // std::cout << m_currentTxBytes << ' ' << m_targetBytes << std::endl;
            ...
            // std::cout << to_write << ' ' << m_currentTxBytes << ' ' << m_targetBytes << std::endl;
            int ret = 0;
            if (to_write > 0)
            {
        ...
        {
            std::cout << m_currentTxBytes << std::endl;
            m_socket->Close();
        }
    }
    ...
    void TcpClient::TraceTxCallback(Ptr<const Packet> packet, const TcpHeader &header, Ptr<const TcpSocketBase> base)
    {
        ...
        // std::cout << m_totalTxBytes << std::endl;
