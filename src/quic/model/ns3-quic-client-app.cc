...
namespace ns3
{
    ...
    {
    public:
        QuicClientAppImpl(QuicClientApp *outer, quic::BackendType backend_type, quic::CongestionControlType cc, uint64_t bytes);
    ...
    static quic::CongestionControlType GetCongestionType(const char *cc_name)
    {
        ...
        else if (0 == strcmp(cc_name, "cubic"))
        {
            cc = quic::kCubicBytes;
        }
        return cc;
    ...
    QuicClientApp::QuicClientApp(quic::BackendType backend_type, const char *cc_name, uint64_t bytes)
    {
        // Chage 4

        m_impl.reset(new QuicClientAppImpl(this, backend_type, GetCongestionType(cc_name), bytes));
    }
    ...
    void QuicClientApp::SendToNetwork(const char *buffer, size_t sz)
    {
        ...
        if (m_impl && m_impl->m_quicClient)
        {
            ...
            // // std::cout << m_socket->GetTxAvailable() << std::endl;
            // int ret = 0;
            // std::cout << sz << ' ' << m_currentTxBytes << ' ' << 1500000000 << std::endl;
            m_currentTxBytes += sz;
            if (m_currentTxBytes < 1500000000)
            {
    ...
    QuicClientAppImpl::QuicClientAppImpl(QuicClientApp *outer, quic::BackendType backend_type, quic::CongestionControlType cc, uint64_t bytes) : m_outer(outer),
                                                                                                                                                 m_cc(cc)
    {
        m_targetBytes = bytes;
        // std::cout << "m_targetBytes : " << m_targetBytes << std::endl;
    ...
    {
        // std::cout << " I am Write Packet Fuction" << std::endl;
