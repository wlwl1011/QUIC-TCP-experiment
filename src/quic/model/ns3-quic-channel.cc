...
namespace quic
{
    ...
    HelloBidiChannel::HelloBidiChannel(Ns3TransportStream *stream) : stream_(stream) {}
    ...
    EchoBidiChannel::EchoBidiChannel(Ns3TransportStream *stream) : stream_(stream) {}
    ...
    HelloUnidiWriteChannel::HelloUnidiWriteChannel(Ns3TransportStream *stream) : stream_(stream) {}
    ...
    HelloUnidiReadChannel::HelloUnidiReadChannel(Ns3TransportStream *stream) : stream_(stream) {}
    ...
    BandwidthWriteChannel::BandwidthWriteChannel(Ns3TransportStream *stream, Ns3QuicAlarmEngine *engine) : stream_(stream)
    {
    ...
    {
        // std::cout << "StartFillData" << std::endl;
        WriteData();
        QuicTime now = Ns3QuicClock::Instance()->Now();
    ...
    void BandwidthWriteChannel::WriteData()
    {
        ...
        if (stream_)
        {
            // std::cout << "in stream" << std::endl;
            for (int i = 0; i < kWritePackets; i++)
            {
                // 내가 1500으로 나누어서 보내는구나 !
                // 이 부분을 수정하면 되는 것인가?
                // std::cout << i << std::endl;
        ...
        // std::cout << write_bytes_ << std::endl;
        NS_LOG_INFO(seconds << " BandwidthWriteChannel::WriteData " << couter);
    }

    BandwidthReadChannel::BandwidthReadChannel(Ns3TransportStream *stream) : stream_(stream) {}
